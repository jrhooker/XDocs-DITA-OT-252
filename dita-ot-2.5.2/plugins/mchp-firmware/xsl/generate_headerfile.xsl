 ...
 <xsl:template name="gather-content">
  ...
  <xsl:call-template name="enum">
   <xsl:with-param name="tableset"
    select="$nodeset//table[descendant::*[contains(@class, ' struct-d/struct ')]] | $nodeset//table[@otherprops = 'struct_only']"/>
  </xsl:call-template>
...
typedef enum {
  ...
  <xsl:variable name="count"
   select="count($tableset[descendant::*[contains(@class, ' struct-d/dword ')]]/descendant-or-self::*[contains(@class, ' topic/table ')]/*[contains(@class, ' topic/title ')] | $tableset[@otherprops = 'struct_only'])"/>
  <xsl:variable name="tables"
   select="$tableset[descendant::*[contains(@class, ' struct-d/dword ')]]/descendant-or-self::*[contains(@class, ' topic/table ')]/*[contains(@class, ' topic/title ')] | $tableset[@otherprops = 'struct_only']"/>
  <xsl:for-each select="$tables">
   <xsl:variable name="processed_name">
    <xsl:choose>
     <xsl:when test="@otherprops = 'struct_only'">
   ...
   <xsl:choose>    
    <xsl:when test="string-length(ancestor::*[contains(@class, ' topic/table ')]/descendant::*[contains(@class, ' struct-d/struct ')]/@opcode) &gt; 0"><xsl:text> = </xsl:text>
     <xsl:value-of select="ancestor::*[contains(@class, ' topic/table ')]/descendant::*[contains(@class, ' struct-d/struct ')]/@opcode"/></xsl:when>
    <xsl:when test="position() = 1 and contains($enum_starting_value, '0x')"><xsl:text> = </xsl:text>
  ...
  <xsl:text>}</xsl:text>
  <xsl:variable name="title" xml:space="default">
   <xsl:call-template name="get-file-title"><xsl:with-param name="title" select="normalize-space(/bookmap/topic[1]/descendant::headerfile-name[1])"/></xsl:call-template>
  </xsl:variable> 
  <xsl:value-of select="lower-case($title)"/><xsl:text>_opcodes_e</xsl:text>
  <xsl:text>;</xsl:text>
 </xsl:template>
...
*/
  ...
  <xsl:for-each select="$tableset/descendant::*[contains(@class, ' struct-d/name ')][not(contains(., 'reserve') or contains(., 'Reserve'))][preceding-sibling::*[contains(@class, ' struct-d/value ')][not(contains(. ,':'))]]">
 ...
 <xsl:template name="define-msb">
  ...
  <xsl:value-of select="upper-case(.)"/><xsl:text>_MSB </xsl:text>
 ...
 <xsl:template name="process-message-name">
  <xsl:param name="name"/>
  <xsl:choose>
   <xsl:when test="contains($name, 'reserve') or contains($name, 'Reserve')">
    <xsl:variable name="temp">
 ...
 <xsl:template name="chapter-header-values">
  ...
  <xsl:variable name="title" select="normalize-space(/bookmap/topic[1]/descendant::headerfile-name[1])"/>
  <xsl:text disable-output-escaping="yes">
#ifndef _</xsl:text><xsl:value-of select="translate(upper-case($title),' ', '_')"/>_MESSAGES_H_<xsl:text>
#define _</xsl:text><xsl:value-of select="translate(upper-case($title),' ', '_')"/>_MESSAGES_H_
 ...
 <xsl:template name="closing-endif">  
  <xsl:variable name="titles" select="normalize-space(/bookmap/topic[1]/descendant::headerfile-name[1])"/>
  <xsl:text disable-output-escaping="yes">#endif /*</xsl:text><xsl:value-of select="translate(upper-case($titles),' ', '_')"/>_MESSAGES_H_<xsl:text>*/</xsl:text>
 </xsl:template>
 ...
 <xsl:template name="calculate-reserved-field-position">
  ...
  <xsl:text>DW</xsl:text><xsl:value-of select="translate($dword, ':', '_')"/><xsl:text>B</xsl:text><xsl:value-of select="$bit_value"/>
 </xsl:template>
